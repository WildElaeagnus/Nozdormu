 ...
 Loop, read, C:\Keep_on\Nozdormu\Nozdormu\NozdormuSettings.txt
{
	...
	If( InStr( A_LoopReadLine, "Время включения:") != 0){ 
				
		TimeString := RegExReplace(A_LoopReadLine, "Время включения: ")
		
	}
	If( InStr( A_LoopReadLine, "Активный плейлист: ") != 0){
		PlaylistLink := RegExReplace(A_LoopReadLine, "Активный плейлист: ")
	}
} 
...
IfMsgBox, Ok
	Secs = -1 ;костыль который кидает в конец отсчета если вдруг захочется ускорить процесс запуска

if( Secs <= 0  ){
 ;тело скрипта
	Run %PlaylistLink%
	WinActivate, Spotify
	send #{Up}	;нажать вин + вверх
	Send {Home} ;на случай если плейлист был прокручен вниз
	Sleep 10000 ;SET 10 secs
	MouseClick, left,  712,  268 ;навести мышку на кнопу плей и нажать ЛКМ
	return
  }
else
	ExitApp
